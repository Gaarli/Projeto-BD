/* =======================================================================================
   Arquivo: esquema.sql
   Descrição: Script DDL para criação da base de dados do sistema EletroReverso.
   Arquitetura: O script segue estritamente a ordenação topológica (Topological Sort).
   As tabelas foram organizadas em "Níveis" de dependência de chaves estrangeiras (FK),
   garantindo que tabelas "pai" sejam criadas antes das tabelas "filhas".
======================================================================================= */

-- =======================================================================================
-- 1. Tabelas Base (Nível 0 - Entidades Fortes)
-- Estas tabelas não possuem dependência de chaves estrangeiras. Servem como os 
-- domínios de dados (dicionários e entidades fundamentais) para o resto do sistema.
-- =======================================================================================

-- Classificação geral dos dispositivos (ex: Linha Branca, Linha Verde).
...
-- Origem física dos resíduos.
-- Justificativa Arquitetural: Utilização de Chave Primária Composta baseada 
-- em atributos naturais (Rua, Cidade, CEP, Estado) em vez de uma chave artificial (ID).
...
-- Centros intermediários onde ocorre a separação manual dos lotes por categoria.
...
-- Centros finais onde ocorre a extração da matéria-prima.
-- Justificativa de Negócio: A restrição UNIQUE em LicencaAmbiental garante 
-- o compliance legal do sistema, impedindo que dois centros tentem operar 
-- com o mesmo documento fraudado ou duplicado.
...
-- Entidade parceira responsável pelo fluxo de frete.
...
-- Catálogo base de elementos químicos/físicos a serem extraídos (ex: Cobre, Lítio).
...
-- Indústrias compradoras que reinserem o material no mercado (Economia Circular).
...
-- =======================================================================================
-- 2. Nível 1 (Dependências Simples)
-- =======================================================================================

-- Mapeia os produtos de prateleira que podem ser descartados, atrelados à sua categoria.
...
-- Tabela Superclasse da Hierarquia de Transporte (Generalização).
-- Justificativa Arquitetural: Concentra os atributos comuns (Datas, Placa, Rastreio)
-- a todos os tipos de viagens logísticas, evitando redundância de dados no banco.
...
-- =======================================================================================
-- 3. Nível 2 (Especializações e Lote Primário)
-- =======================================================================================

-- Tabela Subclasse (Especialização do Transporte para a 1ª perna da viagem).
-- Ação Referencial Otimizada: O uso de ON DELETE CASCADE garante que, se o transporte 
-- genérico pai for excluído, sua especialização morre junto, mantendo a integridade da herança.
...
-- Tabela Subclasse (Especialização do Transporte para a 2ª perna da viagem).
-- Restrição: Exige o preenchimento de um MTR (Manifesto de Transporte de Resíduos) único.
...
-- Unidade operacional principal gerada nos Pontos de Coleta.
-- Restrição de Integridade (UNIQUE): A Chave Natural Composta garante que o sistema 
-- não permita a inserção acidental do mesmo lote duas vezes no mesmo endereço e na mesma data.
-- Ação Referencial Segura (ON DELETE SET NULL): Se um transporte for cancelado, o lote 
-- não é apagado do banco; ele apenas perde a associação do transporte e volta para o estoque (NULL).
...
-- =======================================================================================
-- 4. Nível 3 (Detalhes N:M e Entidades Fracas)
-- =======================================================================================

-- Tabela Associativa: Resolve o relacionamento de "muitos-para-muitos" entre 
-- o Lote e o Catálogo de Dispositivos, registrando o conteúdo exato do caminhão.
...
-- Entidade Fraca: Um Lote Triado não existe sozinho, ele é um pedaço físico 
-- do Lote Coleta original. A chave primária reflete isso, incorporando o ID do Lote Pai.
...
-- =======================================================================================
-- 5. Nível 4 e 5 (Fim do Ciclo Físico)
-- =======================================================================================

-- Registro histórico das ações de desmonte/química realizadas nos centros.
-- Chave Composta (Data, Tipo) permite que um mesmo lote sofra múltiplos processos, 
-- desde que sejam de tipos ou em datas diferentes.
...
-- Fechamento da Rastreabilidade Logística. Associa a extração física ao material 
-- catalogado e ao destinatário final do produto.
